$(function() {
	...
	$('#liaoTian').on('click', function(){
		$('#chatModel').modal({backdrop: false, keyboard: false}).modal('show');
	});



	$('#importContent_but').bind('keypress', function(event) {
		if (event.keyCode == "13") {
			let jQuery = $('#importContent_but').val();
			if(jQuery == ''){
				alert("消息不能为空");
				return;
			} else {
				str = '<div class="atalk"><span>A说 :' + jQuery +'</span></div>';
				console.log("进入回车方法");
				$('#importContent_but').val('');
